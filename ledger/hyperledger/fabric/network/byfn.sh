...
function networkUp() {
  ...
  if [ "${IF_COUCHDB}" == "couchdb" ]; then
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_E2E -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH up -d 2>&1
  else
    IMAGE_TAG=$IMAGETAG docker-compose -f $COMPOSE_FILE_E2E -f $COMPOSE_FILE up -d 2>&1
  fi
  if [ $? -ne 0 ]; then
...
function upgradeNetwork() {
  docker inspect -f '{{.Config.Volumes}}' orderer.blockchain.org | grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.blockchain.org
  docker cp -a orderer.blockchain.org:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.blockchain.org
  docker-compose $COMPOSE_FILES up -d --no-deps orderer.blockchain.org

  for PEER in peer0.org1.blockchain.org peer1.org1.blockchain.org; do
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/org1.blockchain.org/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# block - ``OneOrgsOrdererGenesis`` - and one for our channel - ``OneOrgsChannel``.
# These headers are important, as we will pass them in as arguments when we create
# our artifacts.  This file also contains two additional specifications that are worth
# noting.  Firstly, we specify the anchor peers for each Peer Org
# (``peer0.org1.blockchain.org``).  Secondly, we point to
