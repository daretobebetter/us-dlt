...
function writeRootFabricCA {
   ...
   for ORG in $ORGS; do
      initOrgVars $ORG
      CAPORT="${PREFIXPORT}054"
      writeRootCA
      PREFIXPORT=$((PREFIXPORT+1))
   done
}
...
function writeStartFabric {
   ...
   for ORG in $ORDERER_ORGS; do
	 ...
	 ORDERERPORT="${PREFIXORDERERPORT}050"
         initOrdererVars $ORG $COUNT
         writeOrderer
	 PREFIXORDERERPORT=$((PREFIXORDERERPORT+1))
         COUNT=$((COUNT+1))
      done
   done
   PREFIXPORT=7
   ...
   for ORG in $PEER_ORGS; do
	 ...
	 PEERPORT="${PREFIXPORT}051"
	 COUCHDBPORT="${PREFIXCOUCHDBPORT}984"
         writePeer
	 PREFIXPORT=$((PREFIXPORT+1))
	 PREFIXCOUCHDBPORT=$((PREFIXCOUCHDBPORT+1))
         COUNT=$((COUNT+1))
      done
   done
}
...
"
   echo "  $PEER_NAME:
    ...
    environment:
      ...
      - CORE_LEDGER_STATE_COUCHDBCONFIG_PASSWORD=Blockchain#1
      - FABRIC_CA_CLIENT_HOME=$MYHOME
      - FABRIC_CA_CLIENT_TLS_CERTFILES=$CA_CHAINFILE
