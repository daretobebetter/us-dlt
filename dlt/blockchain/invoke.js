...
var peer = fabric_client.newPeer('grpcs://' + env.settings.DLT_IP + ':7051', { 'pem': Buffer.from(peerCert).toString(), 'ssl-target-name-override': env.settings.PEER });
console.log('grpc://' + env.settings.DLT_IP + ':7051');
console.log(peer);
channel.addPeer(peer);
var order = fabric_client.newOrderer('grpcs://' + env.settings.DLT_IP + ':7050', { 'pem': Buffer.from(ordererCert).toString(), 'ssl-target-name-override': env.settings.ORDERER });
	...
	invoke: function (obj) {
		...
		return (
			Fabric_Client.newDefaultKeyValueStore({ path: store_path }).then((state_store) => {
				...
				//debug: console.log('Asset - ckeck point A :> ' + JSON.stringify(obj));
				...
				//debug: console.log('Asset - ckeck point B :> ' + JSON.stringify(obj) );
